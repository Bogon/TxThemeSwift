    ...
    fileprivate func _setupThemeNotification() {
        if hasAddPickerNotification == false {
            ...
            if #available(iOS 9.0, tvOS 9.0, *) {
                NotificationCenter.default.addObserver(self, selector: #selector(_updateTheme), name: .ThemeUpdateNotification, object: nil)
            } else {
                NotificationCenter.default.addObserver(forName: .ThemeUpdateNotification, object: nil, queue: nil, using: { [weak self] notification in self?._updateTheme() })
            }
        }
    }
